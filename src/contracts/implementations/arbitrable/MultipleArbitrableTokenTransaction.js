...
class MultipleArbitrableTokenTransaction extends Arbitrable {
  ...
  getData = async arbitrableTransactionId => {
    ...
    return {
      seller: arbitrableTransaction[1],
      buyer: arbitrableTransaction[2],
      amount: arbitrableTransaction[3].toNumber(),
      timeout: arbitrableTransaction[4].toNumber(),
      disputeId: arbitrableTransaction[5].toNumber(),
      arbitrator: arbitrableTransaction[6],
      arbitratorExtraData: arbitrableTransaction[7],
      sellerFee: this._Web3Wrapper.fromWei(arbitrableTransaction[8], 'ether'),
      buyerFee: this._Web3Wrapper.fromWei(arbitrableTransaction[9], 'ether'),
      lastInteraction: arbitrableTransaction[10].toNumber(),
      status: arbitrableTransaction[11].toNumber(),
      metaEvidence: await this.getMetaEvidence(arbitrableTransactionId),
      evidence: await this.getEvidence(
        arbitrableTransaction[6],
        arbitrableTransaction[5].toNumber()
      )
    }
  }
}
